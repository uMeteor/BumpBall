...
{
    //说明
    //若使用对象为预制体之列不唯一对象，则不能使用
    /*
    接受信息一方
    private void Awake()
    {
        EventCenter.AddListener(EventType.typeName, delegate);
    }
    private void OnDestroy()
    {
        EventCenter.RemoveListener(EventType.typeName, delegate);
    }

    发送信息一方
    public void ShowInf()
    {
        EventCenter.Broadcast(EventType.typeName);
    }*/
    ...
    {
        Delegate d;
        if (m_EventTable.TryGetValue(eventType, out d))//得到EventType对应的委托
        {
            CallBack callBack = d as CallBack;//类型强转
            if (callBack != null)
            {
                callBack();
            }
